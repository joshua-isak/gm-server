...
def ping(server, ip, data):        # PING
    if (info.debug > 0):
        print(color.cyan + "{}: _Ping!".format(ip) + color.end)
    server.socket.sendto(data, ip)
    print(str(server.port))


def handshake(server, ip, data):   # HANDSHAKE
...
def message(server, ip, data):     # MESSAGE
    print("{}: {}".format(ip, data.decode('utf-8')))
    send_all(server, ip, data)
...
def send_all(server, ip, data):     # SENDALL
    for x in info.clients:
        server.socket.sendto(data, x)